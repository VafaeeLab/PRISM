...
# ============================================================
# Pre-process Clinical and Omics Data (UCEC)
# ============================================================

# Description:
# This section prepares the clinical and omics data for TCGA UCEC. It processes clinical data, including the creation 
...
dim(clin_UCEC_data)  # Check dimensions of the clinical data
...
common_samples <- Reduce(intersect, list(UCEC_GE_sample[[1]], 
                                         UCEC_CNV_sample[[1]], 
                                         UCEC_Meth_sample[[1]], 
                                         UCEC_ME_sample[[1]]))

# ============================================================
# Pre-process Gene Expression Data (UCEC)
# ============================================================

# Description:
# This code handles the pre-processing of TCGA UCEC gene expression data (FPKM values), 
...
# Key Steps:
# 1. Data Download & Preparation: Downloads and prepares TCGA UCEC gene expression data.
...
tcga_UCEC_GE <- GDCprepare(query_UCEC_RNA_TP)  # Prepare UCEC gene expression data
dim(tcga_UCEC_GE)  # Check dimensions (number of features and samples)
colnames(colData(tcga_UCEC_GE))  # Check column names (includes both clinical and expression data)
...
high_var_genes <- UCEC_matrix_GE_filtered[gene_variances >= variance_threshold, ]  # Select high variance genes
# Check how many genes remain after filtering by variance
dim(high_var_genes)
# Retrieve gene metadata (gene names)
UCEC_gene_metadata <- as.data.frame(rowData(tcga_UCEC_GE))
...
UCEC_GE$case_id <- gsub('-01.*', '', UCEC_GE$case_id)  # Remove sample suffix for uniformity
...
UCEC_RNA <- UCEC_GE_data[,-1]  # Remove gene_id column
UCEC_RNA <- UCEC_GE_data[,-2]  # Remove case_id column
UCEC_RNA <- na.omit(UCEC_RNA)  # Remove rows with missing values
...
# ============================================================
# Pre-process Copy Number Variation Data (UCEC)
# ============================================================

# Description:
# This code handles the pre-processing of TCGA UCEC copy number variation (CNV) data. 
...
# Key Steps:
# 1. Data Download & Preparation: Downloads and prepares TCGA UCEC CNV data.
# 2. Missing Data Handling: Filters out genes and samples with excessive missing values.
...
tcga_UCEC_CNV <- GDCprepare(query_UCEC_CNV)  # Prepare UCEC CNV data
dim(tcga_UCEC_CNV)  # Check dimensions (number of features and samples)
...
variance_threshold <- quantile(gene_variances, probs = 0.95)  # Use the 95th percentile as the variance threshold
high_var_genes_indices <- which(gene_variances >= variance_threshold)  # Identify genes with high variance
UCEC_matrix_CNV_filtered <- UCEC_matrix_CNV_filtered[gene_variances >= variance_threshold, ]  # Select high variance genes
...
# Pre-process: Take only samples that are shared by all omics data (common samples)
UCEC_CNV$case_id <- gsub('-01.*', '', UCEC_CNV$case_id)  # Remove sample suffix for uniformity
...
# Clean and reshape the data: Remove unnecessary columns and handle missing values
UCEC_CNV <- UCEC_CNV_data[,-1]  # Remove gene_id column
UCEC_CNV <- UCEC_CNV_data[,-2]  # Remove case_id column
UCEC_CNV <- na.omit(UCEC_CNV)  # Remove rows with missing values
# Reshape data: Convert long format to wide format (genes as columns)
...
# ============================================================
# Pre-process DNA Methylation Data (UCEC)
# ============================================================

# Description:
# This code handles the pre-processing of TCGA UCEC DNA methylation data using the Illumina Human Methylation 27 platform. 
...
# Key Steps:
# 1. Data Download & Preparation: Downloads and prepares TCGA UCEC DNA methylation data (27k probes).
...
# Prepare data for TCGA UCEC methylation (both 27k and 450k platforms)                                                                          
tcga_UCEC_Meth27 <- GDCprepare(query_UCEC_Meth27)
tcga_UCEC_Meth <- GDCprepare(query_UCEC_Meth)
dim(tcga_UCEC_Meth)  # Check dimensions (features and samples)
...
dim(UCEC_matrix_METH_filtered)  # Check the size of the filtered data
# Impute missing values (if any) by filling with the row mean
...
high_var_genes <- UCEC_matrix_METH_filtered[gene_variances >= variance_threshold, ]  # Filter high-variance probes
...
UCEC_METH_data <- UCEC_METH %>% filter(case_id %in% common_samples)
...
UCEC_METH_data <- cbind(
  UCEC_METH_data[, 1],  # Keep case_id in the first column
  UCEC_METH_data[, (num_cols - 1):num_cols],  # Move the last two columns (clinical data) to the front
  UCEC_METH_data[, -c(1, (num_cols - 1):num_cols)]  # Keep the remaining columns (methylation data)
)
...
# ============================================================
# Pre-process MiRNA Data (UCEC)
# ============================================================

# Description:
# This code handles the pre-processing of TCGA UCEC miRNA expression data. It filters out miRNAs and samples 
...
# Key Steps:
# 1. Data Download & Preparation: Downloads and prepares TCGA UCEC miRNA expression data (reads per million).
...
dim(tcga_UCEC_ME)  # Check dimensions (features and samples)
...
tcga_UCEC_ME_columns <- ifelse(seq_along(tcga_UCEC_ME_columns) == 1, tcga_UCEC_ME_columns, 
                               gsub('^reads_per_million_miRNA_mapped_', "", tcga_UCEC_ME_columns))
names(tcga_UCEC_ME_data) <- tcga_UCEC_ME_columns
# Convert data to matrix
UCEC_matrix_ME <- as.matrix(tcga_UCEC_ME_data)
dim(UCEC_matrix_ME)  # Check dimensions of the data
...
dim(UCEC_matrix_ME_filtered)  # Check dimensions after imputation
...
UCEC_ME_data <- UCEC_ME %>% filter(case_id %in% common_samples)
...
filter_ME <- UCEC_ME[, -c(1:3)]  # Exclude the first three columns (case_id, clinical data)
...
dim(UCEC_ME_filtered)  # Check dimensions after filtering
